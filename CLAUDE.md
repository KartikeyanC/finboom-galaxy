...
Live as of 2026-09-07: `20260827120000_trackers.sql` and `20260827130000_trackers_reviewed_at.sql`
are applied to `ludbntvhagefadfkhrjj`, `types.ts` is regenerated (it carries the `trackers` table,
so `dataExport.test.ts` passes), and `feat/trackers` is merged to `master`. The two temporary
shims the migration was blocking are resolved: `src/hooks/trackersClient.ts` is deleted and
`tracker_id` is back on `TransactionInput`. One dead no-op remains — the `withTracker` cast in
`src/hooks/useTransactions.ts`, which only existed to defeat `RejectExcessProperties` while the
column was missing from the generated types — safe to delete on the next touch of that file.

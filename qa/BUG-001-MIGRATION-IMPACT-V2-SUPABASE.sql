-- ===========================================================================
-- BUG-001 v2 — pre-apply impact queries.  READ-ONLY.  (Supabase SQL Editor)
--
-- This is qa/BUG-001-MIGRATION-IMPACT-V2.sql with the psql `\echo` meta-commands
-- removed (the web SQL Editor does not support them). Every SELECT is identical.
...
-- DROP / TRUNCATE.
--
-- Tip: run one Qn block at a time to get a clean result grid per query.
--
...
--   Q8  post-apply only: snapshot-schema grant check
--
-- Report COUNTS where a section says so. Do not paste name / amount / notes.
-- ===========================================================================


-- === Q0  schema state (expect income_stream_id_exists = f) ===
SELECT EXISTS (
...
-- === Q2  AMBIGUOUS — name matches a stream but NOT a confident twin (UNTOUCHED) ===
WITH confident AS (
...
-- === Q3  UNMATCHED — no stream in the tenant shares the name (UNTOUCHED; possible orphan OR manual) ===
SELECT ri.tenant_id, ri.id AS recurring_id, ri.frequency, ri.subtype,
...
-- === Q4  manual-vs-orphan signal summary for Q2 + Q3 rows ===
SELECT
...
-- === Q8  post-apply only: snapshot-schema must expose nothing ===

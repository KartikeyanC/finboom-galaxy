...
export default function AddInvestmentDialog({
  ...
  useEffect(() => {
    ...
    if (initial && seededIdRef.current !== initial.id) {
      ...
      setLinkedAccountId(fields._accountId ?? "none");
      const { _accountId: _removed, ...restFields } = fields;
      void _removed;
      setForm(restFields);
  ...
  const handleSave = () => {
    ...
    const payload: InvestmentRecord = {
      ...
      fields: linkedAccountId && linkedAccountId !== "none"
        ? { ...form, _accountId: linkedAccountId }
        : form,
              ...
              {BROKERS.map((b) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setBroker(b)}
                    className={`px-3 py-2 text-xs rounded-lg border transition-colors text-center truncate ${
                      active

...
/**
 * The sign belongs outside the symbol (-₹100, not ₹-100), and a figure we
 * cannot compute is shown as "—" rather than "₹NaN": a wrong number that looks
 * like a number is worse than an honest blank.
 */
export function formatMoney(amount: number, currency: string = "INR") {
  ...
  const sign = n < 0 ? "-" : "";
  return `${sign}${sym}${Math.abs(n).toLocaleString("en-IN", {
